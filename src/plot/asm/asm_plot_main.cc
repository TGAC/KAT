...
int asmPlotStart(int argc, char *argv[])
{
    ...
    if (!args.ignoreAbsent)
        absentstr << "'" << args.mx_arg->c_str() << "' u 2 t \"Absent\", ";

    std::ostringstream plotstr;
    plotstr << "plot " << absentstr.str() << \
                    "'" << args.mx_arg->c_str() << "' u 3 t \"Distinct\", " \
                    "'" << args.mx_arg->c_str() << "' u 4 t \"Duplicate\", " \
                    "'" << args.mx_arg->c_str() << "' u 5 t \"Triplicate\", " \
                    "'" << args.mx_arg->c_str() << "' u 6 t \"Quadruplate\"";
