...
int kat::PlotProfile::readRecord(std::ifstream& stream, string& id, string& counts) {
    std::string line;
 ...
 **/
int kat::PlotProfile::getEntryFromFasta(const path& fasta_path, const string& header, string& sequence) {
 ...
 **/
int kat::PlotProfile::getEntryFromFasta(const path& fasta_path, uint32_t n, string& header, string& sequence) {
    // Setup stream to sequence file and check all is well
    std::ifstream inputStream (fasta_path.c_str());
