...
//  remove_tmpfiles();
    ...
    if (pclose(gnucmd) == -1)
#endif
        std::cerr << "Problem closing communication to gnuplot" << endl;
}
...
{
    //std::cerr << "showonscreen() functionality removed so that we can run on servers\n";
...
void Gnuplot::init()
{
    // Don't try to get the DISPLAY variable... we will never use X11.
...
/*#if ( defined(unix) || defined(__unix) || defined(__unix__) ) && !defined(__APPLE__)
    ...
    //set terminal type
    showonscreen();       //  Commented this line out because it presumes the DISPLAY variable is set, which might not be the case on a server
